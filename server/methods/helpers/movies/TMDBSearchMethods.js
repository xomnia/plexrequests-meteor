...
Meteor.methods({
  ...
  releaseDates:function(id){
	check(TMDBSearch.api, String);
    check(TMDBSearch.language, String);
	var datesResponse = [];
	var datesResponse = HTTP.call("GET", "https://api.themoviedb.org/3/movie/" + id + "/release_dates?api_key=" + TMDBSearch.api, {} );
	var digitalDate = 0;
	var dvdDate = 0;
	var dates = {"digitalDate":digitalDate, "dvdDate":dvdDate};
	var size = datesResponse.data.results.length;
	
	if (size>0) {
		for (i=0; i < size; i++) {
			var country = datesResponse.data.results[i].iso_3166_1;
			if (country === "US") {
				var dates_size = datesResponse.data.results[i].release_dates.length;
				for (j = 0; j < dates_size ; j++) {
					var datetype = datesResponse.data.results[i].release_dates[j].type;
					if (datetype === 4) {
						digitalDate = datesResponse.data.results[i].release_dates[j].release_date;
						digitalDate = digitalDate.slice(0,10);
					  }
					if (datetype === 5) {
						dvdDate = datesResponse.data.results[i].release_dates[j].release_date;
						dvdDate = dvdDate.slice(0,10);
					  }
				}
			}
		}
		dates = {"digitalDate":digitalDate, "dvdDate":dvdDate};
		return dates
	}else {
		return dates
	}
