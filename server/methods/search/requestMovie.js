...
Meteor.methods({
	"requestMovie": function(request) {
		...
		var other_release_dates=[];
		var other_release_dates=Meteor.call('releaseDates', request.id);
		...
		function insertMovie(request, imdbid, dlStatus, approvalStatus, poster) {
			try {
				Movies.insert({
					...
					poster_path: poster,
					dvd_date: other_release_dates.dvdDate,
					digital_date: other_release_dates.digitalDate
				});
			} catch (error) {
