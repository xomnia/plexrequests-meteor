...
Template.requests.helpers({
  ...
  'dvd_release' : function () {
	if (this.dvd_date==="0"){
		return "Est. : "+moment(this.released).add(3, 'months').format('MMMM Do, YYYY');
	} else {
		return moment(this.dvd_date).format('MMMM Do, YYYY');
	}
  },
  
  'digital_release' : function () {
	if (this.digital_date==="0"){
		return "Est. : "+moment(this.released).add(3, 'months').format('MMMM Do, YYYY');
	} else {
		return moment(this.digital_date).format('MMMM Do, YYYY');
	}
  },
...
Template.requests.events({
  ...
  'click .get-dates' : function (event, template) {
      var request = this;
	  Meteor.call("getDates", request);
  },
